...
⚠️  FOR $9 MICRO-CAPITAL: Use copythatpay_survival.py instead
    The survival mode is a lean, directional micro-scalper designed for
    tiny accounts. It removes hedging complexity and focuses on execution
    discipline with taker entries and fast exits.
...
MAX_CONCURRENT_SYMBOLS    = 6       # fewer symbols, more contracts each
MAX_EXPOSURE_PER_SYMBOL   = 0.15    # 15% of equity per symbol notional
MAX_NOTIONAL_PER_CONTRACT = 1.00    # $1.00 max notional per contract (was 0.01 - too restrictive)
...
ELITE_MODE          = True      # If True, enables elite discipline filters for high-EV trades

# — Spread-capture params —
MIN_REAL_SPREAD        = 0.004    # 0.4% minimum spread to trade (only trade real edge)
SPREAD_TARGET_PCT      = 0.003    # 0.3% target per leg (smaller wins = achievable exits)
MAKER_TIMEOUT_SEC      = 20       # switch to taker after this
QUOTE_REFRESH_SEC      = 5        # re-place quotes when book moves
BOOK_DRIFT_THRESHOLD   = 0.001    # 0.1% drift triggers re-quote
CYCLE_INTERVAL_SEC     = 2        # seconds between symbol cycles

# — Elite mode discipline filters —
ELITE_MIN_SPREAD       = 0.0035   # 0.35% minimum spread (only trade real edge)
ELITE_MIN_MOMENTUM     = 0.001    # 0.1% minimum momentum (meaningful movement)
ELITE_CHOP_MIN         = -0.001   # Chop lower bound (dead zone)
ELITE_CHOP_MAX         = 0.001    # Chop upper bound (dead zone)
ELITE_TREND_KILL       = -0.002   # Trending kill-switch (adverse selection)
ELITE_TARGET           = 0.0045   # 0.45% target (higher quality, fewer trades)
ELITE_ADVERSE_EXIT     = 0.002    # 0.2% adverse selection exit (hard stop loss)
ELITE_COOLDOWN_SEC     = 60       # 60s cooldown after loss
ELITE_HEDGE_THRESHOLD  = 0.01     # Only hedge after 1% profit
ELITE_DEAD_HOURS       = [2, 3, 4, 5]  # UTC dead hours (low liquidity)
ELITE_MIN_EV           = 0.002    # 0.2% minimum expected value
ELITE_EDGE_RATIO_PAUSE = 0.8      # Pause if edge ratio < 80%

# — Execution risk controls (new) —
PANIC_EXIT_SEC         = 60       # flatten stale positions (was 300 — too slow in crypto) - market exit if no fill
...
MAX_ADVERSE_MOVE       = 0.002    # 0.2% max adverse move before forced exit
...
class CopyThatPay:
    ...
    def __init__(self):
        ...
        # Elite mode tracking
        self.last_trade_loss: Dict[str, bool] = {}  # symbol -> was last trade a loss
        self.elite_paused: bool = False  # global pause if edge ratio too low
...
Return ONLY a JSON list of symbols in priority order. Example: ["BTC/USDT:USDT", "ETH/USDT:USDT"]"""
    ...
    async def place_maker_quote(self, symbol: str, side: str, ct: int) -> Optional[str]:
        ...
        try:
            ...
            # Use postOnly unless FORCE_TAKER_MODE is enabled (for testing)
            order_params = {'marginMode': MARGIN_MODE, 'type': 'swap'}
            if not FORCE_TAKER_MODE:
                order_params['postOnly'] = True
                mode_str = 'MAKER'
            else:
                mode_str = 'TAKER (forced)'

            order = await self.exchange.create_limit_order(symbol, side, ct, px, order_params)
            short_sym = symbol.split('/')[0]
            console.print(f'[cyan][{short_sym}] {mode_str} QUOTE {side.upper()} {ct}ct @ {px:.8f} [Order ID: {order.get("id", "N/A")}]')
            return order.get('id', '')
        except Exception as e:
    ...
    async def _manage_side(self, symbol: str, side: str, qs: QuoteState, momentum: float = 0.0) -> None:
        ...
        if filled and entry > 0 and cts > 0:
                ...
                try:
                    ...
                    if order['status'] == 'closed':
                        ...
                        if is_long:
                            ...
                            # Record trade exit for analytics
                            try:
                                notional = entry * cs * cts
                                fees = notional * FEE_RATE * 2  # Fees on notional, not profit
            ...
            if not pos_data or pos_data['cts'] <= 0:
                ...
                qs.cycles += 1
                qs.realized += profit
                self.perf.record_trade_pnl(profit)  # Record after updating state
                short_sym = symbol.split('/')[0]
                h = await self.get_health()
                snap = self.risk.get_snapshot(h['equity'], h['used'], h['free'], self.hedges)
                ...
                if pos_data and pos_data['cts'] > 0:
                    ...
                    try:
                        ...
                        if self.is_adverse_fill(entry_price, current_price, side):
                            ...
                            if side == 'long' and current_price < entry_price * (1 - MAX_ADVERSE_MOVE):
                                console.print(f'[bold red][{short_sym}] ADVERSE EXIT: price dropped {MAX_ADVERSE_MOVE:.1%}')
                                await self.close_position_market(symbol, 'long', pos_data['cts'])
                                return
                            if side == 'short' and current_price > entry_price * (1 + MAX_ADVERSE_MOVE):
                                console.print(f'[bold red][{short_sym}] ADVERSE EXIT: price rose {MAX_ADVERSE_MOVE:.1%}')
    ...
    async def spread_capture_cycle(self, symbol: str) -> None:
        ...
        try:
            ...
            # DIAGNOSTIC MODE: Skip all filters except safety checks
            if DIAGNOSTIC_MODE:
                short_sym = symbol.split('/')[0]
                console.print(f'[bold yellow][DIAGNOSTIC] {short_sym}: Skipping all filters, forcing order placement')
                # Force both sides for testing
                ...
                # ═══════════════════════════════════════════════════════════════════════════════
                # ADAPTIVE MICROSTRUCTURE ENGINE: Edge detection and asymmetric quoting
                # ═══════════════════════════════════════════════════════════════════════════════
                ...
                # Calculate edge score and allowed sides
                edge_score, allowed_sides, edge_reasons = self.calculate_edge_score(imbalance, momentum)

                # PRE-ENTRY FILTERS: Skip dead markets
                try:
                    ...
                    spread_pct = (ask - bid) / ((bid + ask) / 2) if bid and ask else 0

                    # Skip if spread too tight (use MIN_REAL_SPREAD constant)
                    if spread_pct < MIN_REAL_SPREAD:
                        return

                    # Skip if volume too low (< $10k 24h)
                    ...
                    return  # Skip on any data error

                # NO TRADE ZONE: Skip dead or too volatile markets
                if abs(momentum) < 0.0005:
                    short_sym = symbol.split('/')[0]
                    console.print(f'[dim][{short_sym}] DEAD MARKET (momentum={momentum:.4f}) → skipping')
                    return
                if abs(momentum) > 0.01:
                    short_sym = symbol.split('/')[0]
                    console.print(f'[dim][{short_sym}] TOO VOLATILE (momentum={momentum:.3f}) → skipping')
                    return

                # DO NOTHING mode: No edge = No trade
                if edge_score < 2 and not self.hedges.get(symbol):
                    short_sym = symbol.split('/')[0]
                    console.print(f'[dim][{short_sym}] NO EDGE (score={edge_score}): {", ".join(edge_reasons) if edge_reasons else "weak signals"} → skipping')
                    return
            
            # Inventory control: max net exposure per symbol
            hp = self.hedges.get(symbol)
            if hp:
            ...
            if edge_score >= 2:
                short_sym = symbol.split('/')[0]
                sides_str = '/'.join(allowed_sides) if allowed_sides else 'none'
                console.print(f'[cyan][{short_sym}] EDGE (score={edge_score}): {", ".join(edge_reasons)} → quoting: {sides_str}')
    ...
    async def run(self) -> None:
        ...
        try:
            ...
            while True:
                ...
                # LLM symbol selection refresh (every 5 min)
                LLM_REFRESH_SEC = 300
                if time.time() - self.llm_last_update > LLM_REFRESH_SEC:
                    candidates = await self.fetch_eligible_symbols()
                    if candidates:
                        selected = await self.llm_select_symbols(candidates)
                        self.active_symbols = selected
                        self.llm_last_update = time.time()
                        console.print(f'[magenta]LLM SELECTED: {selected}[/magenta]')
                ...
                # Use LLM-selected symbols if available
                active = getattr(self, 'active_symbols', None) or symbols
                console.print(f'[dim]Processing {len(active)} symbols: {active[:3]}...')
                for sym in active:
                    try:
